...
class OrderService:
    ...
    def create_order(
        ...
        try:
            ...
            result = self.orders_repository.create_order(order=order)
            if not result:
                raise Exception("False result when attempting to create order in db")
            return order
        except Exception as ex:
            print(f"Exception {ex} when attempting to create order")
            raise ex

    def update_order_details(self, order_id: str, status: bool, date_of_purchase: datetime) -> UpdateResult:
        result = self.orders_repository.update_order_details(
            order_id=order_id, status=status, date_of_purchase=date_of_purchase
        )
        return result

    def get_order(self, order_id: str):
        result = self.orders_repository.get_order(order_id=order_id)
        return Order(**result)

    def get_orders(self):
        results = self.orders_repository.get_all_orders()
        orders = [Order(**result) for result in results]
        return orders

    def get_orders_by_ticket(self, ticket: str):
        results = self.orders_repository.get_orders_by_ticket(ticket=ticket)
        orders = [Order(**result) for result in results]
        return orders

    def update_order_amount(self, order_id: str, amount: str):
        result = self.orders_repository.update_order_amount(order_id=order_id, amount=amount)
        return result

    def delete_order(self, order_id: str):
        result = self.orders_repository.delete_order(order_id=order_id)
        return result
